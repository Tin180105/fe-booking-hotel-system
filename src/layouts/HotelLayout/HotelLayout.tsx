...
const HotelLayout = () => {
  ...
  return (
    <div className='min-h-screen flex bg-slate-100'>
      <aside
        className={`${collapsed ? 'w-[80px]' : 'w-[260px]'} shrink-0 bg-[#0f5132] text-white transition-all duration-200 flex flex-col`}
      >
        <div className='h-[70px] flex items-center justify-center border-b border-white/10'>
          {collapsed ? (
            <span className='text-[26px] font-bold'>i<span className='text-[#5dc4d4]'>V</span></span>
          ) : (
            <span className='text-[26px] font-bold leading-none text-center'>
              i<span className='text-[#5dc4d4]'>VIVU</span>
              <span className='block text-[13px] font-normal text-white/60'>Hotel</span>
            </span>
          )}
        </div>
      ...
      <div className='flex-1 flex flex-col min-w-0'>
        <header className='h-[70px] bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0'>
          ...
          <div className='flex items-center gap-3'>
            <div className='w-9 h-9 rounded-full bg-[#0f5132] text-white flex items-center justify-center font-semibold'>
              {(profile?.full_name || 'H').charAt(0).toUpperCase()}
            </div>
            <span className='text-[15px] font-medium text-slate-700'>
              {profile?.full_name || 'Quản lý khách sạn'}
            </span>
          </div>
        </header>
